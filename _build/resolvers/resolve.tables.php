...
if ($transport->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
		...
		if(!$payment = $modx->getObject('msPayment',array('name'=>'PayKeeper'))){
			if($payment = $modx->newObject('msPayment')){
				$payment->name = 'PayKeeper';
				$payment->class = 'PayKeeper';
				$payment->active =false;
				$payment->save();
			}
		}
        case xPDOTransport::ACTION_UPGRADE:
            $modelPath = $modx->getOption('msppaykeeper_core_path', null,
                    $modx->getOption('core_path') . 'components/msppaykeeper/') . 'model/';
            $modx->addPackage('msppaykeeper', $modelPath);

            $manager = $modx->getManager();
            $objects = array();
            $schemaFile = MODX_CORE_PATH . 'components/msppaykeeper/model/schema/msppaykeeper.mysql.schema.xml';
